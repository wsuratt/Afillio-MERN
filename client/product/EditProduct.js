...
export default function EditProduct ({match}) {
    ...
    useEffect(() => {
      ...
      read({
        ...
        if (data.error) {
          ...
          setValues({...values, id: data._id, name: data.name, description: data.description, category: data.category, quantity:data.quantity, price: data.price, commission: data.commission})
        }
      })
  ...
  const handleChange = name => event => {
    ...
    if (values.redirect) {
      return (<Redirect to={'/vendor/shop/edit/'+match.params.shopId}/>)
    }
    return (<div>
      <Card className={classes.card}>
        ...
        <CardActions>
          ...
          <Link to={'/vendor/shops/edit/'+match.params.shopId} className={classes.submit}><Button variant="contained">Cancel</Button></Link>
        </CardActions>
      </Card>
    </div>)
}
