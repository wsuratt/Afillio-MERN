...
const list = async (req, res) => {
  try {
    let users = await User.find().select('name email updated created venodr isPending isVendor')
    res.json(users)
  } catch (err) {
    return res.status(400).json({
      error: errorHandler.getErrorMessage(err)
...
const isVending = (req, res, next) => {
  const isVending = req.profile && req.profile.isVendor
  if (!isVending) {
